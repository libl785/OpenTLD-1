...
void FerNNClassifier::trainNN(const vector<cv::Mat> &nn_examples) {
    ...
    for (int i = 0; i < nn_examples.size(); i++) {                          //  For each example
        ...
        if (y[i] == 1 &&
            conf <= thr_nn) {                                //    if y(i) == 1 && conf1 <= tld.model.thr_nn % 0.65
...
void FerNNClassifier::NNConf(const Mat &example, vector<int> &isin, float &rsconf, float &csconf) {
    ...
    for (int i = 0; i < pEx.size(); i++) {
        matchTemplate(pEx[i], example, ncc, TM_CCORR_NORMED);      // measure NCC to positive examples
    ...
    for (int i = 0; i < nEx.size(); i++) {
        matchTemplate(nEx[i], example, ncc, TM_CCORR_NORMED);     //measure NCC to negative examples
